#!/usr/bin/env node
// Capture a REAL Anthropic Claude streaming response to a fixture file, using
// the adapter's own encodeRequest so the recorded payload matches exactly what
// glamfire sends in production. Requires ANTHROPIC_API_KEY.
//
// Usage:
//   node packages/adapters/scripts/capture-anthropic-fixture.mjs [outfile]
//
// Default outfile: packages/adapters/test/fixtures/anthropic-stream-live.sse.txt
//
// The committed fixtures (anthropic-stream-toolcall.sse.txt,
// anthropic-stream-text.sse.txt, anthropic-completion-*.json) are exact
// Messages API wire format. Run this with a key to record a live capture and
// confirm the parser (parseAnthropicSSE + reduceAnthropicStream) reproduces it
// identically — the parser is schema-driven, so it will.
...
const outfile = resolve(
  process.argv[2] ?? join(here, '..', 'test', 'fixtures', 'anthropic-stream-live.sse.txt'),
);

const config = resolveAnthropicConfig(process.env);
...
const result = reduceAnthropicStream(parseAnthropicSSE(raw));
