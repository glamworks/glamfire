...
const ENV_BINDINGS: EnvBinding[] = [
  ...
  { env: 'GLAM_SERVE_PORT', path: ['serve', 'port'], kind: 'number' },
  { env: 'GLAM_SERVE_BIND', path: ['serve', 'bind'], kind: 'string' },
  { env: 'GLAM_SERVE_TARGET', path: ['serve', 'target'], kind: 'string' },
  { env: 'GLAM_SERVE_MODEL', path: ['serve', 'model'], kind: 'string' },
